...
// NewServer creates a new api storage server but does not configure it
func NewServer(api *op.StorageAPI) *Server {
	s := new(Server)
...
type Server struct {
	...
	SocketPath    flags.Filename `long:"socket-path" description:"the unix socket to listen on" default:"/var/run/storage.sock"`
...
func (s *Server) SetAPI(api *op.StorageAPI) {
	if api == nil {
		s.api = nil
		s.handler = nil
		return
